...
-(Guide*) ReserveReifenWechseln
{
    ...
    Step* step = [[Step alloc] initWithName: @"Was Sie brauchen" Description:@"Sie benötigen (v.l.n.r) Reservereifen, Wagenkreuz, Schlitz-Schraubenzieher, Felgenschloss, Wagenheber und optional einen Drehmomentschlüssel. Diese Dinge sollten Sie unter der Kofferraumabdeckung finden." AndImage:image];
    [steps addObject:step];
    
    image = [UIImage  imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"abdeckung" ofType:@"jpg"]];
    step = [[Step alloc] initWithName: @"Reservereifen abmontieren" Description:@"Folgen Sie der Anleitung auf der Kofferraumabdeckung, um den Reservereifen abzumontieren." AndImage:image];
    [steps addObject:step];
    ...
    step = [[Step alloc] initWithName: @"Wagenheber einführen" Description:@"Führen Sie den Wagenheber komplett in die Öffnung ein und kurbeln Sie im Uhrzeigersinn, bis das Rad mindestens 2cm in der Luft hängt." AndImage:image ];
    [steps addObject:step];
    
    image = [UIImage  imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"radab" ofType:@"jpg"]];
    step = [[Step alloc] initWithName: @"Rad abnehmen" Description:@"Schrauben Sie nun die Muttern komplett ab und nehmen Sie das Rad ab. Beim Abnehmen des Rades kann etwas Kraft von Nöten sein." AndImage:image ];
    [steps addObject:step];
    ...
    step = [[Step alloc] initWithName: @"Wagen absenken" Description:@"Senken Sie nun den Wagen ab, indem Sie am Wagenheben gegen den Uhrzeigersinn drehen. Entfernen Sie danach den Wagenheber und stecken die Kappe wieder in die Aussparung." AndImage:image ];
    [steps addObject:step];
    
    image = [UIImage  imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"fest" ofType:@"jpg"]];
    step = [[Step alloc] initWithName: @"Radmuttern festziehen" Description:@"Ziehen Sie nun die Radmuttern mit dem Wagenkreuz fest, wenden Sie Kraft aus aber übertreiben Sie es nicht. Mit dem optionalen Drehmomentschlüssel können Sie die Muttern mit 110 Nm festdrehen." AndImage:image ];
    [steps addObject:step];
